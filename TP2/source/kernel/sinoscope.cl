...
__kernel void kernel_sinoscope(
	__global unsigned char* buffer,
	__global const sinoscope_params_t* sinoscope,
	const unsigned int width,
	const unsigned int height,
	const unsigned int taylor,
	...
	const float px = sinoscope->dx * id_y - 2 * M_PI;
	const float py = sinoscope->dy * id_x - 2 * M_PI;
	...
	color_value(&pixel_value, value, interval, sinoscope->interval_inverse);

	const int index = 3 * (id_y * width + id_x);
