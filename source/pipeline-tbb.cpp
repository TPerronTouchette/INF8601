...
int pipeline_tbb(image_dir_t* image_dir) {
    ...
    auto load_filter = tbb::make_filter<void, image_t*>(tbb::filter::serial, loadFilter(image_dir));
    ...
    auto save_filter = tbb::make_filter<image_t*, void>(tbb::filter::serial_out_of_order, saveFilter(image_dir));
