...
int pipeline_tbb(image_dir_t* image_dir) {
    ...
    // tbb::filter_t load_filter = tbb::make_filter<void, image_t*>(tbb::filter::serial,
    //     [&](tbb::flow_control& fc)-> image_t*{
    //         image_t* imageTemp = image_dir_load_next(image_dir);
    //         return imageTemp;
    //     });

    //     tbb::filter_t scale_filter = tbb::make_filter<image_t*,image_t*>(tbb::filter::parallel,
    //     [](image_t* image){
    //         image_t* tempImg = filter_scale_up(image, 2);
    //         image_destroy(image);
    //         return tempImg;
    //     });

    // tbb::filter_t sobel_filter = tbb::make_filter<image_t*,image_t*>(tbb::filter::parallel,
    //     [](image_t* image){
    //         image_t* tempImg = filter_sobel(image);
    //         image_destroy(image);
    //         return tempImg;
    //     });

    // tbb::filter_t save_filter = tbb::make_filter<image_t*, void>(tbb::filter::serial_out_of_order,
    //     [&](image_t* image){
